...
export default class extends Controller {
    static targets = ['input', 'output', 'status', 'suggestions', 'fontSizeIndicator', 'deepSearchModal', 'deepSearchLoading', 'deepSearchContent']
...
*Cliquez sur **Analyser** pour générer une synthèse approfondie avec l'IA.*`;
    ...
    async onDeepSearch() {
        ...
        // Ouvrir le modal et afficher le chargement
        this.deepSearchModalTarget.classList.remove('hidden');
        this.deepSearchLoadingTarget.classList.remove('hidden');
        this.deepSearchContentTarget.classList.add('hidden');
        this.deepSearchContentTarget.innerHTML = '';
        ...
        try {
            ...
            if (result.success && result.data) {
                this.renderDeepSearchResults(result.data);
            } else {
                this.deepSearchContentTarget.innerHTML = '<div class="col-span-full py-12 text-center text-xs text-red-500">Erreur lors de la recherche approfondie.</div>';
                this.deepSearchLoadingTarget.classList.add('hidden');
                this.deepSearchContentTarget.classList.remove('hidden');
            }
        } catch (error) {
            console.error('Deep search error:', error);
            this.deepSearchContentTarget.innerHTML = '<div class="col-span-full py-12 text-center text-xs text-red-500">Erreur réseau lors de la recherche.</div>';
            this.deepSearchLoadingTarget.classList.add('hidden');
            this.deepSearchContentTarget.classList.remove('hidden');
        }
    }
    ...
    closeDeepSearch() {
        this.deepSearchModalTarget.classList.add('hidden');
    }

    renderDeepSearchResults(articles) {
        this.deepSearchLoadingTarget.classList.add('hidden');
        this.deepSearchContentTarget.classList.remove('hidden');

        if (!articles || articles.length === 0) {
            this.deepSearchContentTarget.innerHTML = '<div class="col-span-full py-12 text-center text-xs text-slate-400">Aucun résultat trouvé sur internet.</div>';
            return;
        }

        // On réutilise la même structure d'articles pour le stockage des résultats dans la liste
        this.deepSearchResultsList = articles;

        this.deepSearchContentTarget.innerHTML = articles.map((article, index) => {
            const isVerified = article.verified === true;
            const verifBadge = isVerified
                ? `<span class="cursor-help text-xs flex-shrink-0" title="Source vérifiée dans les bases de données">✅</span>`
                : `<span class="cursor-help text-xs flex-shrink-0" title="Non vérifié dans les bases de données">⚠️</span>`;
            ...
            return `
            <div class="bg-white p-5 rounded-2xl border ${ isVerified ? 'border-emerald-100' : 'border-slate-100'} shadow-sm hover:border-djoliba/30 transition-all group flex flex-col h-[220px]">
                <div class="flex items-start justify-between mb-2 gap-2">
                ...
                <div class="mt-auto space-y-2">
                    <div class="flex justify-between items-center">
                        <span class="text-[9px] font-mono text-slate-400 truncate max-w-[150px]">${article.doi || (article.url ? 'Lien disponible' : 'Pas de DOI')}</span>
                        ${doiLink}
                    </div>
                    <button data-action="click->literature#addDeepSearchResultToProject" 
                            data-literature-index-param="${index}"
                            class="w-full py-2 bg-slate-50 hover:bg-djoliba text-djoliba hover:text-white text-[10px] font-bold rounded-lg transition-colors border border-slate-200 hover:border-djoliba flex justify-center items-center gap-1">
                        <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
                        Ajouter au projet
    ...
    async addDeepSearchResultToProject(event) {
        const button = event.currentTarget;
        const index = event.params.index;
        const article = this.deepSearchResultsList ? this.deepSearchResultsList[index] : null;

        if (!article) {
            console.error("Article de recherche non trouvé à l'index:", index);
            return;
        }
